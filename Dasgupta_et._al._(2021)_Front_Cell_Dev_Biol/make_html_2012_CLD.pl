...
# miRDeep2 make-HTML script: modified lines 272, 2989, 3641, 3644, 3645, 3651, 3652, 3653, 3654, 3665, 3668, 3869 to allow bowtie to build Rfam index files in the current working directory
...
To get a list of possible species names type make_html_2012_CLD.pl -u
...
sub PrintKnownnotfound{
	...
	## only performed when option z is NOT given, so when the standard make_html_2012_CLD.pl call is done
    if(not $options{'z'}){
...
sub check_Rfam{
    ...
    ## bowtie index is placed in the indexes folder in current working directory
	my $tmp;
    ...
    if(not -f "indexes/Rfam_index.1.ebwt"){
        $err = `bowtie-build $options{'r'} indexes/Rfam_index`;
		if(not -f "indexes/Rfam_index.1.ebwt"){
			print STDERR "$err\n\nRfam index could not be created in indexes/\nPlease check if miRDeep2 is allowed to create the directory indexes/ and write to it
The Rfam analysis will be skipped as long as this is not possible\n\n";
    ...
    if($options{'q'}){
        $err = `bowtie -f -v 1 -a --best --strata --norc indexes/Rfam_index expression_analyses/expression_analyses_${time}/identified_precursors.fa expression_analyses/expression_analyses_${time}/rfam_vs_precursor.bwt`;
        ...
        $err = `bowtie -f -v 1 -a --best --strata --norc indexes/Rfam_index mirdeep_runs/run_${time}/identified_precursors.fa mirdeep_runs/run_${time}/rfam_vs_precursor.bwt`;
...
sub Usage{
	print STDERR "\n\n\n\n[usage]\n\tperl make_html_2012_CLD.pl -f miRDeep_outfile [options]\n\n";
