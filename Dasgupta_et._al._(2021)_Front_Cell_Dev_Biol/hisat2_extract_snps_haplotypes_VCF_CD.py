...
#                                                                       #
#  Customized to work with Ensembl VCF files that contain additional    #
#  annotation categories (e.g. tandem repeats). As stated in the        #
#  distributed code, "Given a VCF line, the function reports a list of  #
#  variants [pos, type, data] [reporting] 'S' for single nucleotide     #
#  polymorphism, 'D' for deletion, and 'I' for insertion." The          #
#  distributed python code is unable to interpret the tandem repeat     #
#  information included in the Ensembl VCF file. Attempts to extract    #
#  snps and haplotypes from the Ensembl GRCz10.91 VCF file with the     #
#  original script resulted in assertion errors. Instead of stopping,   #
#  this modified code will skip lines in the VCF file that do not       #
...
def extract_vars(chr_dic, chr, pos, ref_allele, alt_alleles, varID): 
    ...
    for a in range(len(alt_alleles)):
		...
		#added above line of code to skip lines containing tandem repeat info
        assert min_len >= 1
        if min_len > 1:
        ...
        if len(ref_allele2) == 1 and len(alt_allele) == 1:
            type = 'S'
            data = alt_allele
            if ref_allele2 == alt_allele: continue
			#added above line of code to skip lines containing tandem repeat info
