...
## last update 2016/04/21 ##CLD modified lines 237, 372, 387 and 392, inserted 388 and 391 on 2019/01/03
...
## create a log file for the mapper_CLD_Q33.pl
## the latest run of mapper_CLD_Q33 will be on top of the log file
##
##
...
sub map_reads{
	...
	print MAP "bowtie --phred33-quals --chunkmbs 1000 --threads $threads -f --seedmms $mismatches_seed --maqerr 80 --seedlen 18 -m $mapping_loc --best -k $mapping_loc $file_genome_latest  --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped  $file_reads_latest $dir/mappings.bwt 2> mapper_logs/${prefix}_bowtie_alignment_precheck.log\n\n";
	print MAP "${prefix}_bowtie_alignment_precheck.log does not contain final alignment counts and values, but will allow you to check for any glaring inconsistencies in your read mapping while the mapper is still analyzing samples. Final alignment values can be found at the end of the mapper standard error file, which I route to a UNIQUE_IDENTIFIER_mapper_report.log file\n\n";
	#bowtie -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest $file_reads_latest $dir/mappings.bwt\n\n";

	if(not -d "mapper_logs"){mkdir "mapper_logs";}
	my $ret_mapping=`bowtie --phred33-quals --chunkmbs 1000 --threads $threads -f --seedmms $mismatches_seed --maqerr 80 --seedlen 18 -m $mapping_loc --best -k $mapping_loc $file_genome_latest  --al $dir/${orig_file_reads}_mapped --un $dir/${orig_file_reads}_not_mapped  $file_reads_latest $dir/mappings.bwt 2> mapper_logs/${prefix}_bowtie_alignment_precheck.log`;
