...
# miRDeep2 perl script: modified to replace make_html.pl with make_html_2012_CLD.pl so Rfam bowtie index files can be created in the current directory
...
miRDeep2_2012_CLD.pl reads genome mappings miRNAs_ref/none miRNAs_other/none precursors/none [options] 2>report.log
...
miRDeep2_2012_CLD.pl reads.fa genome.fa reads_vs_genome.arf mautre_ref_miRNAs.fa mature_other_miRNAs.fa  hairpin_ref_miRNAs -t Mouse 2>report.log
...
# foreach my $e(@ARGV){
# 	if($e =~ /(~)/ or $e =~ /(\s)/){
# 		die "*******\nYour call to miRDeep2_2012_CLD.pl has an argument $e with allowed characters or whitespace in its name $1 . Please make sure not to have '~' or any whitespace in your arguments\n******\n\n";
# 	}
...
my $warning="\n**********\nThe first three arguments to miRDeep2_2012_CLD.pl must be files while arguments 4-6 can be files or must be designated as 'none'. Examples:\n
miRDeep2_2012_CLD.pl reads.fa genome.fa reads_vs_genome.arf mautre_ref_miRNAs.fa mature_other_miRNAs.fa  hairpin_ref_miRNAs

or

miRDeep2_2012_CLD.pl reads.fa genome.fa reads_vs_genome.arf none none none


Please check if the supplied files exist and the command call to miRDeep2_2012_CLD.pl is correct
*******************\n";
...
Please make sure that you provided the correct file and the correct parameter ordering when calling miRDeep2_2012_CLD.pl
...
sub output_results{
    ...
    if($file_mature_ref_this_species !~ /none/i){
        ...
        if($options{'q'}){
            $line="/nfs1/EMT/Tanguay_Lab/Bioinfo/dunhamc/Scripts/make_html_2012_CLD.pl -f $dir/output.mrd -k $dir_tmp/$file_mature_ref_this_species -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -q $options{'q'} -x $xopt -r ${scripts}/../Rfam_for_miRDeep.fa -v $sc -y $time $sort_by_sample $OE";
        }else{
            $line="/nfs1/EMT/Tanguay_Lab/Bioinfo/dunhamc/Scripts/make_html_2012_CLD.pl -f $dir/output.mrd -k $dir_tmp/$file_mature_ref_this_species -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -r ${scripts}/../Rfam_for_miRDeep.fa -v $sc -y $time  $sort_by_sample $OE";
        }
    }else{
        if($options{'q'}){
            $line="/nfs1/EMT/Tanguay_Lab/Bioinfo/dunhamc/Scripts/make_html_2012_CLD.pl -f $dir/output.mrd -p $dir_tmp/precursors.coords -s $dir/survey.csv -c -e -q $options{'q'}  -x $xopt -r ${scripts}/../Rfam_for_miRDeep.fa -v $sc -y $time $sort_by_sample $OE";
        }else{
            $line="/nfs1/EMT/Tanguay_Lab/Bioinfo/dunhamc/Scripts/make_html_2012_CLD.pl -f $dir/output.mrd -p $dir_tmp/precursors.coords -v $sc -s $dir/survey.csv -c -e -r ${scripts}/../Rfam_for_miRDeep.fa -y $time $sort_by_sample $OE";
        }
    }
...
sub test_first_argument{
    ...
    if($ARGV[0] eq '-u'){system("/nfs1/EMT/Tanguay_Lab/Bioinfo/dunhamc/Scripts/make_html_2012_CLD.pl -u -y 1"); exit;}
